...
function result = fourthB(Ts, coeffs, Tol)
addpath('..');
if (nargin<3)
    Tol = 1e-5;
end
...
for rp=1:nbeta
  ...
  B4A = integral3(@(r1, r2, r3)(mfun(r1, bcoeffs).*mfun(r2, bcoeffs) ...
      .*mfun(r3-r2, bcoeffs).*mfun(r3-r1, bcoeffs).*(mfun(r3, bcoeffs) ...
      .*(mfun(r2-r1, bcoeffs)+6) + 3)), ...
      0, upperr, 0, upperr, 0, upperr, ...
      'AbsTol',1e-2,'RelTol',Tol,'method','iterated'); %
  B4C = integral3(@(r1, r2, r3)(mfun(r1, bcoeffs).*mfun(r2, bcoeffs) ...
      .*mfun(r3-r1, bcoeffs).*mfun(r3-r2, bcoeffs)), ...
      0, upperr, @(r1)(r1), upperr, upperr, @(r1, r2)(r2+upperr), ...
      'AbsTol',1e-2,'RelTol',Tol,'method','iterated'); % 
  
  B4s(rp) = -0.5*(B4A+3*B4C);   % 4*(-1/8)*(B4A+6*B4B+3*B4C)
end
if(nargin<2)
