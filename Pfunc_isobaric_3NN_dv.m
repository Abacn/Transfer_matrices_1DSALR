...
if dPoint > sigma
    ...
    dsA = (dPoint-sigma)/divideA;
    dsB = (kappasigma-dPoint)/(divideB-1);
    dsBA = dsB/dsA;
    dsBAlist = [ones([divideA,1]); ones([divideB-1, 1]).*dsBA; 1];
    rlistA = linspace(sigma, dPoint-dsA, divideA).'+dsA/2;
    rlistB = linspace(dPoint, kappasigma, divideB).' + dsB/2;
    rlist = [rlistA; rlistB];
    optdflag = true;
    ds = dsA;
else
